...
class MCPClient:
    ...
    async def _run(self,query: str):
        ...
        server_parameters = StdioServerParameters(
            command = "python",  #server文件可以是 .py， .js
            args=[self.server_path] #服务端脚本文件的代码路径
        )
        #2. 创建读写流通道
        async with stdio_client(server=server_parameters) as (read_stream, write_stream):
        #3. 创建客户端与服务端进行通信的session对象
            # session = ClientSession(read_stream,write_stream) 改进一下以应用垃圾清理机制↓
            async with ClientSession(read_stream,write_stream) as session:
                ...
                if choice.finish_reason == "tool_calls":
                    #大模型选择了工具的情况
                    # 为了后期，大模型能更加精准的恢复，把大模型选择的工具的message，重写添加到messages中
